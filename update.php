...
<form action="man.php?action=update" id="add_form" method="post">
	...
	<tr><td width = "85"><label for="channel_number">渠道号</labvel></td>
		<td width = "200"><input type="text" class="xax" id="channel_number" name="channel_number" value="<?php echo $qop["channel_number"]; ?>" disabled = "true"/></td>
		<td class='tip' data_tip="channel_number"></td>
	</tr>
	<tr><td><label for="channel_name">渠道名称</labvel></td>
		<td><input type="text" class="xax" id="channel_name" name="channel_name" value="<?php echo $qop["channel_name"]; ?>" maxlength="32"/></td>
		<td class='tip' data_tip="channel_name"></td>
	</tr>
	<tr><td><label for="bd">BD</labvel></td>
		<td><input type="text" class="xax" id="bd" name="bd" value="<?php echo $qop["bd"] ?>" maxlength="32"/></td>
		<td class='tip' data_tip="bd"></td>
	</tr>
	<tr><td><label for="promotion_team">推广团队</labvel></td>
		<td>
			<select id="promotion_team" name="promotion_team" size="1" class="xax" disabled = "true">
				<option value="">--请选择--</option>
				<?php 
					$rows=$db->selectPromotionTeams();
					foreach($rows as $row){
						if($row["promotion_number"]==$qop["promotion_number"])
							echo "<option selected value='{$row["promotion_number"]}' >{$row["team_name"]}</option>";
						else 
	...
	<tr><td><label for="payment_method">付费方式</labvel></td>
		<td>
			<select id="payment_method" name="payment_method" size="1" class="xax"  disabled = "true">
				<option value="">--请选择--</option>
				<?php 
					$rows=$db->selectPaymentMethods();
					foreach($rows as $row){
						if($row["payment_number"]==$qop["payment_number"])
							echo "<option selected value='{$row["payment_number"]}'>{$row["payment_name"]}</option>";
						else
	...
	<tr><td><label for="cooperation_mode">合作方式</labvel></td>
		<td>
			<select id="cooperation_mode" name="cooperation_mode" size="1" class="xax"  disabled = "true">
				<option value="">--请选择--</option>
				<?php 
					$rows=$db->selectCooperationModes();
					foreach($rows as $row){
						if($row["cooperation_number"]==$qop["cooperation_number"])
							echo "<option selected value='{$row["cooperation_number"]}'>{$row["cooperation_name"]}</option>";
						else
	...
	<tr><td><label for="version_type">版本类型</labvel></td>
		<td>
			<select id="version_type" name="version_type" size="1" class="xax" disabled = "true">
				<option value="">--请选择--</option>
				<?php 
					$rows=$db->selectVersionTypes();
					foreach($rows as $row){
						if($row["version_number"]==$qop["version_number"])
							echo "<option selected value='{$row["version_number"]}'>{$row["type_name"]}</option>";
						else
	...
	<tr><td><label for="has_sdk">有无SDK</labvel></td>
		<td>
			<select id="has_sdk" name="has_sdk" size="1" class="xax">
				...
				<?php 
					...
					foreach($rows as $row){
						if($row["sdk_number"]==$qop["has_sdk"])
							echo "<option selected value='{$row["sdk_number"]}'>{$row["type_name"]}</option>";
						else
	...
	<textarea  class="button" id="edit_des" name="description" cols="90" placeholder="<?php echo $qop["description"]?>"></textarea><br>
...
(function($){	
	...
	$("#submit").click(function(e){
		...
		$.post("man.php?action=update",$("#add_form").serialize(),function(data){
			if(data == 1){
